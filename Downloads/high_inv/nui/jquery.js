...
const updateDrag = () => {
	...
	$('.use').droppable({
		...
		drop: function(event,ui){
			...
			$.post("http://high_inv/useItem", JSON.stringify({
				item: itemData.key,
				type: itemData.type,
	...
	$('.drop').droppable({
		...
		drop: function(event,ui){
			...
			$.post("http://high_inv/dropItem", JSON.stringify({
				item: itemData.key,
				amount: Number($("#amount").val())
			}))
	...
	$('.send').droppable({
		...
		drop: function(event,ui){
			...
			$.post("http://high_inv/sendItem", JSON.stringify({
				item: itemData.key,
				amount: Number($("#amount").val())
			}))
...
const updateMochila = () => {
	document.getElementById("amount").value = "";
	$.post("http://high_inv/requestMochila",JSON.stringify({}),(data) => {
		...
		$('#invleft').html(`
			...
			${nameList.map((item) => (`
				<div class="item" style="background-image: url('images/${item.index}.png');" data-item-key="${item.key}" data-item-type="${item.type}" data-name-key="${item.name}">
